...
public class VitessReplicationConnectionIT {
    ...
    @Test
    public void shouldHandleExcludeKeyspaceFromTableName() throws Exception {
        // setup fixture
        final VitessConnectorConfig conf = new VitessConnectorConfig(
        ...
        try (VitessReplicationConnection connection = new VitessReplicationConnection(conf, vitessDatabaseSchema)) {
            ...
            // Verify that the table name in the INSERT message does not include the keyspace
            ReplicationMessage insertMessage = messages.get(1).getMessage();
            String tableName = insertMessage.getTable();
            // Interface should not be changed and should contain both keyspace & table name
            assertThat(tableName).isEqualTo("\"0\".\"test_unsharded_keyspace\".\"t1\"");
        }
        finally {
            if (error.get() != null) {
                LOGGER.error("Error during streaming", error.get());
