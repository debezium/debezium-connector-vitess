...
function wait_for_shard_tablets() {
	...
	for _ in $(seq 1 ${wait_secs}); do
		cur_tablets=$(vtctldclient --grpc-auth-static-client-creds grpc_static_client_auth.json GetTablets --keyspace "${keyspace}" --shard "${shard}" | wc -l)
...
function wait_for_healthy_shard_primary() {
	...
	for _ in $(seq 1 ${wait_secs}); do
		if ! vtctldclient --grpc-auth-static-client-creds grpc_static_client_auth.json --server=localhost:15999 GetShard "${keyspace}/${shard}" | grep -qi "${unhealthy_indicator}"; then
			break
		fi
		sleep 1
	done;
...
function wait_for_writeable_shard_primary() {
	...
	PRIMARY_TABLET="$(vtctldclient --grpc-auth-static-client-creds grpc_static_client_auth.json --server=localhost:15999 GetTablets --keyspace "$keyspace" --shard "$shard" | grep -w "primary" | awk '{print $1}')"
	if [ -z "$PRIMARY_TABLET" ] ; then
		fail "Cannot determine primary tablet for keyspace/shard $keyspace/$shard"
	fi

	for _ in $(seq 1 ${wait_secs}); do
		if vtctldclient --grpc-auth-static-client-creds grpc_static_client_auth.json --server=localhost:15999 GetFullStatus "$PRIMARY_TABLET" | grep "super_read_only" | grep --quiet "false" ; then
			break
		fi
		sleep 1
	done
	if vtctldclient --grpc-auth-static-client-creds grpc_static_client_auth.json --server=localhost:15999 GetFullStatus "$PRIMARY_TABLET" | grep "super_read_only" | grep --quiet "true" ; then
...
function wait_for_shard_vreplication_engine() {
        ...
        for _ in $(seq 1 ${wait_secs}); do
                if vtctlclient --grpc-auth-static-client-creds grpc_static_client_auth.json --server=localhost:15999 Workflow -- "${keyspace}" listall &>/dev/null; then
                        break
                fi
                sleep 1
        done;
...
function wait_for_workflow_running() {
    ...
    for _ in $(seq 1 ${wait_secs}); do
        result=$(vtctldclient --grpc-auth-static-client-creds grpc_static_client_auth.json Workflow --keyspace="${keyspace}" show --workflow="${workflow}" 2>/dev/null | grep "Copy phase completed")
        if [[ ${result} != "" ]]; then
            break
        fi
        sleep 1
